...
func printRoute(route *routeapi.Route, options kprinters.PrintOptions) ([]metav1.TableRow, error) {
	row := metav1.TableRow{
	...
	row.Cells = append(row.Cells, name, host, route.Spec.Path, strings.Join(backendInfo, ","), port, policy, route.Spec.WildcardPolicy)

	return []metav1.TableRow{row}, nil
}

func printRouteList(list *routeapi.RouteList, options kprinters.PrintOptions) ([]metav1.TableRow, error) {
