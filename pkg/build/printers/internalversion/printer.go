...
func printBuild(build *buildapi.Build, options kprinters.PrintOptions) ([]metav1.TableRow, error) {
	row := metav1.TableRow{
	...
	row.Cells = append(row.Cells, name, buildinternalhelpers.StrategyType(build.Spec.Strategy),
		from, status, created, duration)

	return []metav1.TableRow{row}, nil
}

func printBuildList(list *buildapi.BuildList, options kprinters.PrintOptions) ([]metav1.TableRow, error) {
...
func printBuildConfig(bc *buildapi.BuildConfig, options kprinters.PrintOptions) ([]metav1.TableRow, error) {
	row := metav1.TableRow{
	...
	if bc.Spec.Strategy.CustomStrategy != nil {
		row.Cells = append(row.Cells, name, buildinternalhelpers.StrategyType(bc.Spec.Strategy),
			bc.Spec.Strategy.CustomStrategy.From.Name, bc.Status.LastVersion)
	} else {
		row.Cells = append(row.Cells, name, buildinternalhelpers.StrategyType(bc.Spec.Strategy), from,
...
func printBuildConfigList(list *buildapi.BuildConfigList, options kprinters.PrintOptions) ([]metav1.TableRow, error) {
	rows := make([]metav1.TableRow, 0, len(list.Items))
	for i := range list.Items {
